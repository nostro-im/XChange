...
public class BinanceExchangeIntegration {
  ...
  protected static void createExchange() throws Exception {
    ...
    boolean useSandbox =
        Boolean.parseBoolean(
            System.getProperty(ExchangeSharedParameters.PARAM_USE_SANDBOX, Boolean.FALSE.toString()));
    spec.setExchangeSpecificParametersItem(ExchangeSharedParameters.PARAM_USE_SANDBOX, useSandbox);
  ...
  protected void assumeProduction() {
	Assume.assumeFalse(
	  "Using sandbox", 
	  (Boolean) exchange.getExchangeSpecification().getExchangeSpecificParametersItem(ExchangeSharedParameters.PARAM_USE_SANDBOX)
	);
  }
